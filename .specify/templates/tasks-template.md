...
- [ ] T010 Enforce desktop UI framework boundary (CustomTkinter-only usage, root/app composition)
- [ ] T011 Enforce provider boundary (no UI-layer external provider or DB I/O imports)
- [ ] T012 Add secrets/config protections (token/API key/path from config, no hardcoded secrets)
- [ ] T013 Define local-first cache and offline fallback behavior (including `fetched_at` visibility)
- [ ] T014 Define source mapping and credibility notes for any non-primary data provider
